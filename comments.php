	...
	if ( have_comments() ) : ?>
		<h2 class="comments-title">
			...
			if ( '1' === $comment_count ) {
				printf(
					/* translators: 1: title. */
					esc_html__( 'One thought on &ldquo;%1$s&rdquo;', 'cyclingclublite' ),
					'<span>' . get_the_title() . '</span>'
				);
			} else {
				printf( // WPCS: XSS OK.
					/* translators: 1: comment count number, 2: title. */
					esc_html( _nx( '%1$s thought on &ldquo;%2$s&rdquo;', '%1$s thoughts on &ldquo;%2$s&rdquo;', $comment_count, 'comments title', 'cyclingclublite' ) ),
		...
		<ol class="comment-list">
			...
			<?php
				wp_list_comments( array(
					'callback' => 'cyclingclublite_comments',
					'style'      => 'ol',
					'short_ping' => true,
				) );
			?>
		...
		if ( ! comments_open() ) : ?>
			<p class="no-comments"><?php esc_html_e( 'Comments are closed.', 'cyclingclublite' ); ?></p>
		<?php
		endif;
